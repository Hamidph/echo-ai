...
class SystemConfigResponse(BaseModel):
    ...
    default_iterations: int = Field(
        ..., description="Default number of iterations for new experiments"
    )
    default_frequency: str = Field(
        ..., description="Default frequency for recurring experiments (daily/weekly/monthly)"
    )
    default_recurring: bool = Field(..., description="Whether recurring is enabled by default")
    max_iterations_per_experiment: int = Field(
        ..., description="Maximum allowed iterations per experiment"
    )
    enable_recurring_experiments: bool = Field(
        ..., description="Whether recurring experiments are enabled platform-wide"
    )
...
class SystemConfigUpdate(BaseModel):
    ...
    default_iterations: int | None = Field(
        None, ge=1, le=100, description="Default number of iterations"
    )
    default_frequency: str | None = Field(
        None, pattern="^(daily|weekly|monthly)$", description="Default frequency"
    )
    default_recurring: bool | None = Field(None, description="Default recurring setting")
    max_iterations_per_experiment: int | None = Field(
        None, ge=1, le=1000, description="Max iterations allowed"
    )
    enable_recurring_experiments: bool | None = Field(
        None, description="Enable/disable recurring experiments"
    )
...
class AdminStatsResponse(BaseModel):
    ...
    active_recurring_experiments: int = Field(
        ..., description="Number of active recurring experiments"
    )
