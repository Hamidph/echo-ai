...
class BrandProfileBase(BaseModel):
    ...
    brand_description: str | None = Field(
        None, max_length=500, description="What your company does"
    )
...
class BrandProfileCreate(BrandProfileBase):
    ...
    brand_competitors: list[str] = Field(
        default_factory=list, max_items=10, description="Competitor names"
    )
    brand_target_keywords: list[str] = Field(
        default_factory=list, max_items=20, description="Target SEO keywords"
    )
